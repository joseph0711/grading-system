...
const AttendancePage = () => {
  usePageTitle("attendanceGrading");
      ...
      const fetchStudents = async () => {
        try {
          const response = await fetch(
            `/api/grading/attendance?courseId=${courseId}`,
            {
              method: "GET",
              credentials: "include",
            }
          ...
          const studentsWithAbsences = data.map((student: any) => ({
            student_id: student.student_id,
            name: student.name,
            absence_times: student.absence_times || 0,
          }));
  ...
  const { handleAutoSave, isSaving } = useAutoSave({
    onSave: async (value: any) => {
      return showToast.promise(
        fetch(`/api/grading/attendance?courseId=${courseId}`, {
          method: "POST",
  ...
  const handleAbsenceChange = (student_id: string, value: string) => {
    const absenceTimes = value === "" ? 0 : parseInt(value, 10);

    // Validate the input value
    if (absenceTimes < 0 || absenceTimes > 10) {
      showToast.error(t.absenceTimesMustBeBetween0And10);
      return;
    }
    ...
    setStudents((prevStudents) =>
      prevStudents.map((student) =>
        student.student_id === student_id
          ? { ...student, absence_times: absenceTimes }
          : student
      )
    );
  ...
  const filteredStudents = students.filter(
    (student) =>
      ...
      student.student_id.toUpperCase().includes(searchTerm.toUpperCase())
  );
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() =>
                                handleAbsenceChange(
                                  student.student_id,
                                  String(Math.max(0, student.absence_times - 1))
                                )
                              }
                              disabled={isSaving || student.absence_times <= 0}
                            ...
                            <input
                              ...
                              onKeyDown={(e) => {
                                // Prevent decimal point, 'e', and minus sign
                                if (
                                  e.key === "." ||
                                  e.key === "e" ||
                                  e.key === "-"
                            ...
                            <button
                              onClick={() =>
                                handleAbsenceChange(
                                  student.student_id,
                                  String(
                                    Math.min(10, student.absence_times + 1)
                                  )
                                )
                              }
                              disabled={isSaving || student.absence_times >= 10}
                              className="w-8 h-8 flex items-center justify-center rounded-md border border-gray-300 dark:border-gray-600 
                                     bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 
                                     hover:bg-gray-50 dark:hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
