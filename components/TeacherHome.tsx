...
import ManageCourse from './ManageCourse';  // Make sure the path is correct for your project structure
import GradingSelection from './grading-selection';  // Import the GradingSelection component

type TeacherHomeProps = {
  onLogout: () => Promise<void>;
};

const TeacherHome: React.FC<TeacherHomeProps> = ({ onLogout }) => {
  const [showManageCourse, setShowManageCourse] = useState(false);
  const [showGradingSelection, setShowGradingSelection] = useState(false);  // State for GradingSelection

  const handleGradingClick = () => {
    setShowGradingSelection(true);  // Set state to true to show GradingSelection component
  };

  const handleManageCourseClick = () => {
    setShowManageCourse(true);  // Set state to true to show ManageCourse component
  };
  ...
  const handleBackToHome = () => {
    setShowManageCourse(false);  // Set state to false to return to TeacherHome
    setShowGradingSelection(false);  // Set state to false to return to TeacherHome
  };
  ...
  return (
    <div className="grid grid-rows-[50px_1fr_50px] min-h-screen p-8 pb-20 gap-16 sm:p-20 bg-gray-50 dark:bg-gray-900 transition-colors">
      ...
      <div className="flex justify-between items-center">
        <div className="text-3xl font-extrabold text-blue-600 dark:text-blue-300">Home Page</div>
        <button
          onClick={onLogout}
      ...
      {showManageCourse ? (
        ...
        <div className="flex flex-col items-center">
          <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-300 mb-8">Teacher Mode</h2>
